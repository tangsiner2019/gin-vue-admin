<template>
  <div>
    ...
    >
      ...
      <el-table-column label="链接地址" prop="url" width=""></el-table-column>
      ...
      <el-table-column label="序号" prop="sort" width="60" align="center"></el-table-column>
    ...
    <el-dialog
      ...
      :title="type === 'create' ? '新增友情链接' : '编辑友情链接'"
    >
      <add-item
...
export default {
  ...
  methods: {
    ...
    async onDelete() {
      ...
      const res = await deleteLinkByIds({ ids });
      if (res.code == 0) {
        this.$message({
          type: "success",
          message: "删除成功",
    ...
    async updateLink(row) {
      const res = await findLink({ ID: row.ID });
      this.type = "update";
      if (res.code == 0) {
        this.formData = res.data.relink;
    ...
    async enterDialog(item) {
      ...
      switch (this.type) {
        case "create":
          res = await createLink(this.formData);
          break;
        case "update":
          this.formData.ID = item.id;
          res = await updateLink(this.formData);
          break;
        default:
          res = await createLink(this.formData);
          break;
